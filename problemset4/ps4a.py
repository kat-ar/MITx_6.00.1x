...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    assert type(word) == str, 'not a word'
    totalScore = 0
    for char in word:
        totalScore += SCRABBLE_LETTER_VALUES[char]
    totalScore = totalScore*len(word)
    if len(word) == n:
        totalScore += 50
    assert totalScore >= 0, 'something in the calculations went wrong'
    return totalScore
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                # Tell the user how many points the word earned, afastnd the updated total score, in one line followed by a blank line
                score = getWordScore(newWord, n)
                print("\""+ str(newWord) + "\" earned " + str(score) + " points.", end=' ')
                totalScore += score
...
def playGame(wordList):
    ...
    while choice != 'e' :
        if choice != 'n' and choice != 'r':
            ...
            choice = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
        # if user wants to end the game, break out of the loop
        elif choice == 'e':
            break
        # if user wants to replay previous hand
        elif choice == 'r':
            # check if there had been a hand played in this instance of game
            if lastHand == {}:
                # if not, ask for correct input
                print("You have not played a hand yet. Please play a new hand first! ")
                choice = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
            else:
                # if yes, play game
                gameHand = lastHand.copy()
                playHand(gameHand, wordList, HAND_SIZE)
                choice = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
        # else if user wants to start with a new hand
        elif choice == 'n':
            # deal new hand
            gameHand = dealHand(HAND_SIZE)
            # remember the hand
            lastHand = gameHand.copy()
            # play game
            playHand(gameHand, wordList, HAND_SIZE)
            choice = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
